namespace CQRS.Middleware
{
    ...
    public class CustomMiddleware33
    {
        private readonly RequestDelegate _next;
        public CustomMiddleware33(RequestDelegate next)
        {
            _next = next;
        }
        public async Task Invoke(HttpContext context)
        {
            ...
            Console.WriteLine(HttpMethod.Get.Method);
            Console.WriteLine(context.Request.Path.Value);
            Console.WriteLine("CustomMiddleware33 " + context.Request.Method + context.Request.Host.Value);
            if (context.Request.Method == HttpMethod.Get.Method)
            {
                Console.WriteLine("This is the get method");
                await _next.Invoke(context);
